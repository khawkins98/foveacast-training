...
| `train.py`          | **Phase 4 landed** | Fine-tuning loop. `--prototype` mode (100 train / 25 val / 2 epochs) is the Phase 4 gate — "does the loss curve look plausible" — and closed cleanly on 2026-04-16 (train 0.92→0.76, val CC 0.60→0.64, ~90 seconds on M4 MPS). Default mode is the Phase 5 full fine-tune (full train set, 30 epochs, placeholder hyperparameters pending Phase 5 tuning). Device auto-detect (`mps → cuda → cpu`). Loss is KL divergence (`src/foveacast_training/losses.py`); validation metric is Pearson correlation coefficient. Writes per-step and per-epoch history to `runs/{mode}-{timestamp}/history.json` (gitignored). | `msinet`, `ueyes_dataset`, `losses`, `torch` |
